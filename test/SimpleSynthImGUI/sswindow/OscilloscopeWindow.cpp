...
void oscilloscopeWindow(CurrentProjectContext & ctx){
	...
	if(ImGui::BeginTable("table1", 2)){
		for(auto &channel : mixer.getAllChannels()){
			...
			ImGui::PlotLines((channel->getName() + " L").c_str(), floatValues[0], length, 0, "", -1.0f, 1.0f, ImVec2(200, 40.0f));
			ImGui::PlotLines((channel->getName() + " R").c_str(), floatValues[1], length, 0, "", -1.0f, 1.0f, ImVec2(200, 40.0f));
		}
		ImGui::EndTable();
	}
