...
namespace yzrilyzr_simplesynth{
	...
	void TwoStringResonator::postProcess(u_sample * input, u_index length){
		for(u_index i=0;i < 8;i++){
	...
	u_sample TwoStringResonator::getAmp(const Note & note){
		...
		IIRUtil::RBJ_biquad(data.hiCut, RBJParams{FilterPassType::LOWPASS, IIRUtil::limitFreq(sampleRate, hcFreq), note.cfg->sampleRate, exciterHiCutQ});
		IIRUtil::RBJ_biquad(data.lowCut, RBJParams{FilterPassType::HIGHPASS, IIRUtil::limitFreq(sampleRate, lcFreq), note.cfg->sampleRate, exciterLowCutQ});
	...
	TwoStringResonatorKeyData * TwoStringResonator::init(TwoStringResonatorKeyData * data, const Note & note){
		...
		data->comb1.fill(0);
		data->comb2.fill(0);
		IIRUtil::RBJ_biquad(data->hiCut, RBJParams{FilterPassType::ALLPASS, 200, note.cfg->sampleRate, 1, 0});
		IIRUtil::RBJ_biquad(data->lowCut, RBJParams{FilterPassType::ALLPASS, 200, note.cfg->sampleRate, 1, 0});
		data->hiCut.init(note.cfg->sampleRate);
		data->lowCut.init(note.cfg->sampleRate);
		data->hiCut.resetMemory();
		data->lowCut.resetMemory();
		return data;
	}
	bool TwoStringResonator::noMoreData(const Note & note)const{
