...
namespace yzrilyzr_simplesynth{
	...
	void PianoSrc2::postProcess(u_sample * input, u_index length){
		soundboard.procBlock(input, input,length);
		//重置合成重复状态
		u_index siz=CHANNEL_MAX_NOTE_ID * sizeof(false);
		memset(isInSynth, false, siz);
	}
	bool PianoSrc2::noMoreData(const Note & note)const{
		int id=note.id;
		if(id < 21 || id>108)return true;
		const PianoKey & pianoKey=pianoKeys[id];
