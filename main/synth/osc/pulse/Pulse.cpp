...
namespace yzrilyzr_simplesynth{
	...
	u_sample Pulse::getAmp(const Note & note){
		return PWM::pwm(note.phaseSynth, width, rise, fall, delay) * note.velocitySynth;
	}
	...
	NoteProcPtr Pulse::clone(){
		return mksp<Pulse>(width, rise, fall, delay);
	}
	String Pulse::toString()const{
		return StringFormat::object2string("Pulse", width, rise, fall, delay);
	}
}