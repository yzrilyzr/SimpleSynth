...
namespace yzrilyzr_simplesynth{
	...
	WaveSampler::WaveSampler(s_phase phaseMul, u_sp<SampleProvider> sampleData, u_index sampleOffset, u_index sampleLength,
	...
	u_sample WaveSampler::getAmp(const Note & note){
		...
		s_phase noteIndex=note.phaseSynth * phaseMul;
	...
	bool WaveSampler::noMoreData(const Note & note)const{
		...
		switch(loopType){
			...
			case LOOP_ONCE:
			default:
				s_phase index=note.phaseSynth * phaseMul;
	...
	String WaveSampler::toString() const{
		return StringFormat::object2string("WaveSampler",
										   phaseMul,
										   sampleData,
										   sampleOffset,
										   sampleLength,
