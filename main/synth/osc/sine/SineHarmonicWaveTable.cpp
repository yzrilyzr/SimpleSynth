...
namespace yzrilyzr_simplesynth{
	...
	SineHarmonicWaveTable::SineHarmonicWaveTable(const Array<DoubleArray> & aa){
		this->aa=aa;
		this->interpolator=mksp<LineInterpolator>();
	}
	...
	u_sample SineHarmonicWaveTable::getAmp(const Note & note){
		return a(note, note.phaseSynth * Math::TAU) * note.velocitySynth;
	}
	...
	yzrilyzr_lang::String SineHarmonicWaveTable::toString() const{
		return yzrilyzr_lang::StringFormat::object2string("SineHarmonicWaveTable", aa);
	}
}