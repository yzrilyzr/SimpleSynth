...
namespace yzrilyzr_simplesynth{
	...
	SineWaveTable::SineWaveTable(double baseFreq, const DoubleArray & freqAndAmp){
		this->aa=freqAndAmp;
		this->baseFreq=baseFreq;
	}
	u_sample SineWaveTable::getAmp(const Note & note){
		return a(note.phaseSynth * Math::TAU, note.cfg->tuning->getFrequencyByID(note.id)) * note.velocitySynth;
	}
